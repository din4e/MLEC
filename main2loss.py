...
class EC:
    ...
    def iterativesecure(self, pi, rho=[]):
        ...
        for i in x:
            if i == 1 or i == 1.0:
                cost = cost + 1
        return x, self.getLambda([], x), cost
    ...
    def HDG(self):
        ...
        pi = [x[0] for x in l]
        rho = list(reversed(pi))
        x, Lambda, _ = self.iterativesecure(pi, rho)
        if not self.isNE(x):
            print("ERROR")
...
class CNN(nn.Module):
    ...
    def forward(self, xb):
        ...
        for j, a in enumerate(x):
            ...
            ec = EC(a, G.Lambda * 0.5)  # 根据邻接矩阵a 获得图的点边信息 以及ECGame相关的参数
            xx, Lambda, cost = ec.iterativesecure(pi, rho)  # Strategy([1, 0, 0, 1, 1])
            if not ec.isNE(xx):
                print("ERROR")
            xb_cost.append(cost)
        ...
        # return xb_is
        xb_cost = torch.Tensor(xb_cost)
        xb_cost = Variable(xb_cost.double(), requires_grad=True)
        return xb, xb_cost
